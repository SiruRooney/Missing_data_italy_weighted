...
EM.BIC=function(predata,bape,ind.bape,rs.ind,wgh){
  ...
  while((i.EM==1)|(max(abs(unlist(bape)-unlist(bape0)))>=10e-5)){
    ...
    i.EM=i.EM+1
    #print(abs(unlist(bape)-unlist(bape0)))
  }
  ...
  BIC.def=function(predata,bape,ind.bape,rs.ind,wgh,comp.ind,mis.type){
    ...
    log.BIC=function(predata,bape,ind.bape,rs.ind,wgh,comp.ind,mis.type){
      ...
      return(list(Q=l.obs,obs=l.obs1))
      #return(l.obs)
    }
    
    bic.l=log.BIC(predata,bape,ind.bape,rs.ind,wgh,comp.ind,mis.type)
    def.val.obs=-2*bic.l$obs+p*log(NROW(predata[comp.ind==TRUE,]))
    def.val.Q=-2*bic.l$Q+p*log(NROW(predata))
    return(list(val.obs=def.val.obs,val.Q=def.val.Q))
  }
  
  bic.mis=BIC.def(predata,bape,ind.bape,rs.ind,wgh,comp.ind,mis.type)
  return(list(par.coef=bape,iter.EM=i.EM,BIC.mis=bic.mis))  
